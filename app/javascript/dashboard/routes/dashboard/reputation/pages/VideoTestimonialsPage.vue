...
const fetchTestimonials = async () => {
  ...
  try {
    const [testRes, reqRes, tempRes] = await Promise.all([
      ...
      axios.get(`/api/v1/accounts/${accountId}/reputation/video_testimonials/requests_index`),
...
const deleteTestimonial = async id => {
  ...
  try {
    await axios.delete(`/api/v1/accounts/${accountId}/reputation/video_testimonials/${id}`);
...
const openModal = async () => {
  ...
  try {
    const tempRes = await axios.get(`/api/v1/accounts/${accountId}/reputation/templates`);
    templates.value = tempRes.data.filter(t => t.active && t.template_type === 'video');
  } catch (err) {
...
const sendRequest = async () => {
  ...
  try {
    await axios.post(`/api/v1/accounts/${accountId}/reputation/video_testimonials/dispatch_request`, payload);
  ...
  <div class="p-6 max-w-7xl mx-auto space-y-6">
    ...
    >
      ...
      <p class="text-xs text-slate-400 mt-1 max-w-sm text-center">
        Share the submission form link with your customers to start collecting video testimonials.
      </p>
    </div>

    <!-- Grid View -->
    <div v-else-if="!loading && testimonials.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
      ...
      >
        ...
        <div class="aspect-video bg-black relative">
          ...
          <div v-else class="w-full h-full flex flex-col items-center justify-center text-slate-500">
            <svg class="size-8 mb-2 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <div class="p-4 flex-1 flex flex-col justify-between space-y-4">
          <div>
            <h3 class="font-extrabold text-slate-800 dark:text-slate-100 text-sm line-clamp-2">
              "{{ t.title }}"
            </h3>
            <p class="text-[11px] text-slate-500 font-medium mt-1">{{ t.email }}</p>
          </div>
          
          <div class="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800">
            <span class="text-[10px] text-slate-400 font-medium">{{ formatDate(t.created_at) }}</span>
            <button
    ...
    <!-- Outbound Logs -->
    <div v-if="!loading" class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-850 shadow-sm overflow-hidden mt-8">
      <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-850">
        <h3 class="font-extrabold text-slate-900 dark:text-white text-sm">Outbound Video Requests</h3>
      </div>
      ...
      <div v-if="requests.length === 0" class="flex flex-col items-center justify-center py-16 space-y-2">
        ...
        <p class="text-sm font-bold text-slate-700 dark:text-slate-355">No video requests dispatched</p>
      </div>
      ...
      <table v-else class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-850">
          <tr v-for="req in requests" :key="req.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-850/30 text-xs transition-colors">
    ...
    >
      <div class="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-2xl p-6 space-y-5">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-850 pb-3">
          <h3 class="text-base font-extrabold text-slate-950 dark:text-white">Send Video Testimonial Request</h3>
          <button 
        ...
        <div class="space-y-4">
          <div class="space-y-1.5">
            <label class="block text-xs font-bold text-slate-450 uppercase tracking-wider">Customer Email</label>
            <input
          ...
          <div class="space-y-1.5">
            <label class="block text-xs font-bold text-slate-450 uppercase tracking-wider">Message Template (Optional)</label>
            <select
          ...
          <div class="space-y-1.5">
            <label class="block text-xs font-bold text-slate-450 uppercase tracking-wider">Email Subject</label>
            <input
          ...
          <div class="space-y-1.5">
            <label class="block text-xs font-bold text-slate-450 uppercase tracking-wider">Message</label>
            <textarea
            ...
            ></textarea>
            <p class="text-[10px] text-slate-400 mt-1">Keep <code class="bg-slate-100 dark:bg-slate-800 px-1 rounded">{{video_link}}</code> in the message, it will be replaced by the real link.</p>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-850">
          ...
          <button
            ...
            @click="sendRequest"
          >
            <svg v-if="sending" class="size-3.5 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 7.89M9 11l3-3 3 3m0 0l-3 3-3-3" /></svg>
            Send Request
          </button>
        </div>
      </div>
