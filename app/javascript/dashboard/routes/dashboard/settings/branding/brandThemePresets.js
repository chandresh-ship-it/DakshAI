/**
 * Brand theme presets — full light/dark token maps from new-ui ThemeSwitcher.
 * Selecting a preset applies every CSS variable (incl. sidebar*), not just 4 swatches.
 * Regenerate: node scripts one-off from Downloads/new-ui/src/config/themes.ts
 */
...
export const findPresetById = id =>
  BRAND_THEME_PRESETS.find(p => p.id === id) || null;
...
export const findMatchingPreset = colors => {
  ...
  return (
    BRAND_THEME_PRESETS.find(p => {
      if (p.primary.toLowerCase() !== primary) return false;
      if (secondary && p.secondary.toLowerCase() !== secondary) return false;
      if (accent && p.accent.toLowerCase() !== accent) return false;
      if (background && p.background.toLowerCase() !== background) return false;
      return true;
    }) || null
  );
};

/** Flatten light tokens + swatches for brand_colors persistence / SSR. */
export const brandColorsFromPreset = (preset, extras = {}) => {
  ...
  return {
    ...
    text: preset.text,
    // Chrome tokens (sidebar etc.) — SSR + reload without waiting on JS catalog alone
    ...light,
    // map new-ui "foreground" onto stored "text" without dropping CSS foreground
